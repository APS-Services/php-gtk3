                ...
                Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
                    [state](HRESULT result, ICoreWebView2Controller *controller) -> HRESULT {
                              ...
                              [state](ICoreWebView2 *sender,
                                ...
                                if (uri) {
                                  int required_size = WideCharToMultiByte(
                                      CP_UTF8, 0, uri, -1, nullptr, 0, nullptr, nullptr);
                                  if (required_size > 0) {
                      ...
                      state->webview->add_NavigationCompleted(
                          Callback<ICoreWebView2NavigationCompletedEventHandler>(
                              [state](ICoreWebView2 *sender,
                                ...
                                if (title) {
                                  int required_size = WideCharToMultiByte(
                                      CP_UTF8, 0, title, -1, nullptr, 0, nullptr, nullptr);
                                  if (required_size > 0) {
                                    std::vector<char> buffer(required_size);
                                ...
                                for (const auto &handler : state->message_handlers) {
                                  ...
                                  if (!is_valid_handler_name(handlerName)) {
                                    g_warning(
                                        "WebView2: Skipping re-injection "
                      ...
                      if (!state->pending_uri.empty()) {
                        int required_size = MultiByteToWideChar(
                            CP_UTF8, 0, state->pending_uri.c_str(), -1, nullptr, 0);
                        if (required_size > 0) {
                          std::vector<wchar_t> w_uri(required_size);
                          MultiByteToWideChar(CP_UTF8, 0, state->pending_uri.c_str(), -1,
                        ...
                        int required_size = MultiByteToWideChar(
                            CP_UTF8, 0, state->pending_html.c_str(), -1, nullptr, 0);
                        if (required_size > 0) {
...
void WebKitWebView_::register_script_message_handler(Php::Parameters &parameters) {
  if (parameters.size() == 0) {
    throw Php::Exception(
        "register_script_message_handler() expects at least 1 "
        "parameter, 0 given");
  }
...
void WebKitWebView_::set_user_data_folder(Php::Parameters &params) {
  ...
  if (state->webview) {
    g_warning(
        "WebView2: set_user_data_folder() must be called before the "
