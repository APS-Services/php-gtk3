...
struct WebView2State {
    ...
    WebView2State() : hwnd(nullptr), is_loading(false) {}
};
...
// Get native Windows HWND
    ...
    std::wstring userDataFolderWide;
    ...
    if (required_size > 0) {
        userDataFolderWide.resize(required_size);
        MultiByteToWideChar(CP_UTF8, 0, folder_path.c_str(), -1, &userDataFolderWide[0], required_size);
        userDataFolder = userDataFolderWide.c_str();
    }
                        ...
                        Callback<ICoreWebView2NavigationCompletedEventHandler>(
                            [state](ICoreWebView2 *sender, ICoreWebView2NavigationCompletedEventArgs *args) -> HRESULT {
                                ...
                                for (const auto &handler : state->message_handlers) {
                                    ...
                                    if (!is_valid_handler_name(handlerName)) continue;  // skip unsafe handler names
