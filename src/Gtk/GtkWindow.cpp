...
Php::Value GtkWindow_::set_icon_from_file(Php::Parameters &parameters) {
  ...
  return gtk_window_set_icon_from_file(GTK_WINDOW(instance), filename.c_str(), nullptr);
}
...
void GtkWindow_::begin_move_drag(Php::Parameters &parameters) {
  if (parameters.size() != 4) {
    throw Php::Exception(
        "begin_move_drag expects 4 parameters: button, "
...
void GtkWindow_::begin_resize_drag(Php::Parameters &parameters) {
  if (parameters.size() != 5) {
    throw Php::Exception(
        "begin_resize_drag expects 5 parameters: edge, "
