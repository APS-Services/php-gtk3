...
Php::Value GtkButton_::new_with_label(Php::Parameters &parameters)
{
	if (!parameters[0].isString()) {
		std::string error_msg = "GtkButton::new_with_label: First parameter (label) must be a string, ";
...
{
	if (!parameters[0].isString()) {
		std::string error_msg = "GtkButton::set_label: First parameter (label) must be a string, ";
		error_msg += phpgtk_type_to_string(parameters[0].type());
		error_msg += " given";
		throw Php::Exception(error_msg);
	}
