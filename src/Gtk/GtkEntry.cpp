...
void GtkEntry_::set_text(Php::Parameters &parameters)
{
	if (!parameters[0].isString()) {
		Php::warning << "GtkEntry::set_text: First parameter (text) should be a string, " 
		            << phpgtk_type_to_string(parameters[0].type()) 
		            << " given (auto-converting)" << std::flush;
	}
	std::string text = parameters[0].stringValue();
...
void GtkEntry_::set_invisible_char(Php::Parameters &parameters)
{
	...
	std::string string = parameters[0].stringValue();
...
{
	std::string new_text = parameters[0].stringValue();
