...
if [ "$1" == "php" ]; then
	shift
	/usr/local/php-gtk3/bin/php -dextension=/app/php-gtk3-source/gtk3.so "$@"
fi
...
if [ "$1" == "build" ]; then
	...
	cp /app/php-gtk3-source/gtk3.so $(php-config --extension-dir)/
	echo "extension=gtk3" >> $(php-config --ini-path)/php.ini
fi

# tests to copy dependencies
if [ "$1" == "ldd" ]; then

	/usr/local/php-gtk3/bin/php -dextension=/app/php-gtk3-source/gtk3.so -m

	/app/copy.ldd.sh /usr/local/php-gtk3/bin/php -dextension=/app/php-gtk3-source/gtk3.so -m
fi